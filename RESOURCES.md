<!-- This file is part of OpenClaw Guide for Beginners. Licensed under the MIT License. See LICENSE file for details. -->

# 推荐资源汇总

> OpenClaw相关的高质量资源和工具链接

---

## 🤖 API平台

| 平台 | 官网文档 | 优惠活动 | 推荐指数 |
|------|---------|---------|----------|
| 硅基流动 | [文档](https://docs.siliconflow.cn) | [注册送2000万Tokens](https://cloud.siliconflow.cn/i/lva59yow) | ⭐⭐⭐⭐⭐ |
| 火山引擎 | [文档](https://docs.volcengine.com) | [折上9折，低至8.9元](https://volcengine.com/L/oqijuWrltl0/  ) | ⭐⭐⭐⭐⭐ |
| 智谱AI | [文档](https://docs.bigmodel.cn) | [GLM Coding限时价](https://www.bigmodel.cn/glm-coding?ic=BUDVTRHUYH) | ⭐⭐⭐⭐ |
| MiniMax | [文档](https://platform.minimaxi.com/docs) | [9折优惠](https://platform.minimaxi.com/subscribe/coding-plan) | ⭐⭐⭐⭐ |

### 邀请码汇总

| 平台 | 邀请码/链接 | 用途 |
|------|------------|------|
| 硅基流动 | `lva59yow` | 注册送2000万Tokens |
| 火山引擎 | `XCWPZTHW` | Coding Plan折上9折 |
| 智谱GLM Coding | 自动追踪 | 限时惊喜价 |

---

## ☁️ 云服务器

| 云服务商 | 教程 | 优惠活动 | 推荐指数 |
|---------|------|---------|----------|
| 阿里云 | [教程](./docs/installation/cloud/aliyun-guide.md) | [99元专属套餐](https://www.aliyun.com/activity/ecs/clawdbot?userCode=yyzsc1al) | ⭐⭐⭐⭐⭐ |
| 腾讯云 | [教程](./docs/installation/cloud/cloud-deployment-guide.md) | [推广大使特惠](https://curl.qcloud.com/JnWPPHIH) | ⭐⭐⭐⭐ |

### 推荐配置

| 用途 | 配置 | 云服务商 | 月费 |
|------|------|---------|------|
| 测试 | 1核1G | 阿里云 | 60-80元 |
| 推荐 | 2核2G | 阿里云 | 8-10元（99元/年） |
| 多用户 | 2核4G | 阿里云 | 12-15元 |
| 重度 | 4核8G | 阿里云 | 30-40元 |

---

## 📚 官方资源

### 核心文档

- [OpenClaw官方文档](https://docs.openclaw.ai)
- [GitHub仓库](https://github.com/openclaw/openclaw)
- [NPM包](https://www.npmjs.com/package/openclaw)
- [更新日志](https://github.com/openclaw/openclaw/releases)

### 社区资源

- [Discord社区](https://discord.gg/clawd)
- [GitHub Discussions](https://github.com/openclaw/openclaw/discussions)
- [Twitter](https://twitter.com/openclaw_ai)

---

## 🛠️ 技能扩展

### 官方技能库

- [ClawHub](https://clawhub.com) - 技能市场
- [官方技能文档](https://docs.openclaw.ai/skills)

### 热门技能

| 技能 | 功能 | 安装命令 |
|------|------|---------|
| weather | 天气查询 | `openclaw skills install weather` |
| healthcheck | 健康检查 | `openclaw skills install healthcheck` |
| feishu | 飞书集成 | `openclaw skills install feishu` |
| self-improvement | 自我提升 | `openclaw skills install self-improvement` |

详见[技能开发与使用](docs/advanced/skills.md)

---

## 🔗 对接平台

### 已支持平台

| 平台 | 教程 | 状态 |
|------|------|------|
| 飞书 | [教程](./docs/platform-int/egration/feishu-integration.md) | ✅ 稳定 |
| 钉钉 | [教程](./docs/integration/dingtalk-integration.md) | ✅ 稳定 |
| 微信（企业） | [教程](./docs/integration/wecom-integration.md) | ✅ 稳定 |
| Telegram | [教程](./docs/integration/telegram-integration.md) | ✅ 稳定 |
| Discord | [教程](./docs/integration/discord-integration.md) | 🚧 开发中 |
| Slack | - | 🚧 开发中 |
| Signal | - | 🚧 开发中 |

---

## 📖 学习资源

### 教程

- [5分钟快速上手](docs/start/quickstart.md)
- [API配置教程](docs/configuration/api-configuration.md)
- [模型选择指南](docs/configuration/model-comparison.md)
- [成本优化指南](docs/configuration/cost-optimization.md)
- [云服务器部署](docs/installation/cloud/cloud-deployment-guide.md)
- [阿里云专属教程](docs/installation/cloud/aliyun-guide.md)

### 进阶指南

- [安全配置指南](docs/advanced/security.md)
- [技能开发与使用](docs/advanced/skills.md)
- [故障排除](docs/advanced/troubleshooting.md)

---

## 💡 相关工具

### 开发工具

| 工具 | 用途 | 链接 |
|------|------|------|
| Node.js | 运行环境 | https://nodejs.org |
| npm | 包管理器 | https://npmjs.com |
| nvm | Node版本管理 | https://github.com/nvm-sh/nvm |
| Docker | 容器化部署 | https://docker.com |

### 辅助工具

| 工具 | 用途 | 链接 |
|------|------|------|
| htop | 系统监控 | https://htop.dev |
| tmux | 终端复用 | https://tmux.github.io |
| git | 版本控制 | https://git-scm.com |

---

## 🔐 安全资源

### 安全最佳实践

- [安全配置指南](docs/advanced/security.md)
- [API Key安全](docs/configuration/api-configuration.md#api-key安全)
- [防火墙配置](docs/installation/cloud/aliyun-guide.md#配置防火墙)

### 安全工具

- [OpenSSL](https://www.openssl.org) - 加密工具
- [SSH Key](https://www.ssh.com/ssh/key) - SSH密钥管理

---

## 📊 监控与分析

### 使用统计

```bash
# 查看今日使用量
openclaw stats today

# 查看本月使用量
openclaw stats month

# 查看模型分布
openclaw stats models
```

### 日志查看

```bash
# 实时日志
openclaw logs -f

# 错误日志
openclaw logs --level error

# 最近100行
openclaw logs -n 100
```

---

## 🆘 获取帮助

### 快速帮助

```bash
# 查看帮助
openclaw --help

# 查看命令帮助
openclaw <command> --help
```

### 问题排查

- [常见问题解答](FAQ.md)
- [故障排除](docs/advanced/troubleshooting.md)
- [GitHub Issues](https://github.com/openclaw/openclaw/issues)

---

## 📈 路线图

### 新手学习路线

```
1. 快速开始 (5分钟)
   └─ quickstart.md

2. API配置
   ├─ api-configuration.md
   ├─ model-comparison.md
   └─ cost-optimization.md

3. 选择部署方式
   ├─ 本地部署 (Linux/macOS/Windows)
   └─ 云服务器部署
       ├─ cloud-deployment-guide.md
       └─ aliyun-guide.md

4. 连接对话平台
   └─ 选择平台并配置

5. 进阶使用
   ├─ security.md
   ├─ skills.md
   └─ troubleshooting.md
```

---

## 📝 更新日志

### 2026-02-22

- ✅ 新增快速开始指南
- ✅ 新增模型选择指南
- ✅ 新增成本优化指南
- ✅ 新增阿里云专属教程
- ✅ 新增常见问题解答
- ✅ 更新返利链接

---

## 🔗 外部链接

### 有用链接

- [OpenAI API文档](https://platform.openai.com/docs)
- [Anthropic Claude文档](https://docs.anthropic.com)
- [飞书开放平台](https://open.feishu.cn/)
- [钉钉开放平台](https://open.dingtalk.com/)
- [Telegram Bot API](https://core.telegram.org/bots/api)

---

**创建时间**: 2026-02-22
**版本**: 1.0
**维护者**: OpenClaw社区

<!-- This file is part of OpenClaw Guide for Beginners. Licensed under the MIT License. See LICENSE file for details. -->
