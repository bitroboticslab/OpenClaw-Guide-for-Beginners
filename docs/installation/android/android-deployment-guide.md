...
## ⚠️ v2026.4.9 安卓配对更新说明
新版本完全重构了安卓配对流程，支持直接扫描QR码快速配对，无需手动输入配对码：
1. 不再支持旧的手动输入6位配对码的方式
2. 旧的配对记录会自动失效，需要重新配对
3. 配对时自动同步配置，无需手动设置服务器地址
...
## 📱 第五步：配对OpenClaw安卓客户端（v2026.4.9+支持）
新版本支持直接通过QR码快速配对，无需手动配置：
1. 在手机上安装OpenClaw官方安卓客户端（从官网下载，无需Google Play）
2. 在Termux中执行命令生成配对QR码：
```bash
openclaw pair --qr
```
3. 用安卓客户端扫描屏幕上显示的QR码
4. 等待10秒自动完成配对，所有配置会自动同步到客户端
5. 配对成功后就可以直接在手机APP上和AI助手对话了

> 💡 提示：如果无法扫描屏幕，可以使用`openclaw pair --link`生成配对链接，在手机上点击链接直接配对。
...
| 火山方舟 | 首月 8.91元 | [订阅](https://volcengine.com/L/oqijuWrltl0/  ) |
