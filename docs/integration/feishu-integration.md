...
### 话题线程配置（v2026.5.5+支持）
✅ v2026.5.5版本修复了话题线程ID路由错误问题，支持回复到对应话题中：

**配置方法**：无需额外配置，升级到v2026.5.5+版本即可自动生效。

**功能特性**：
- 自动识别话题线程ID，回复会准确发送到对应话题下
- 历史话题回复不会错位
- 支持在话题中@机器人，回复保持在话题内

> 💡 如果升级后仍有问题，可执行`openclaw doctor --fix`修复配置即可。
...
**问题原因**：v2026.5.4及之前版本存在话题线程ID路由错误bug。

**解决方案**：
1. 升级OpenClaw到v2026.5.5或更高版本
2. 执行`openclaw doctor --fix`自动修复配置
3. 重启gateway服务：`openclaw gateway restart`
4. 如仍有问题，可在飞书开放平台重新发布应用版本即可。
